...
Interpreter.prototype.initGlobalScope = function(scope) {
  ...
  const func = this.createNativeFunction(x => {
...
Interpreter.prototype.initFunction = function(scope) {
  ...
  wrapper = function(var_args) {
    ...
    let argsStr = Array.prototype.slice
      .call(arguments, 0, -1)
      .join(',')
      .trim();
    if (argsStr) {
    ...
    if (thisInterpreter.isa(separator, thisInterpreter.REGEXP)) {
      ...
      if (thisInterpreter.REGEXP_MODE === 2) {
        if (Interpreter.vm) {
          ...
          const pid = thisInterpreter.regExpTimeout(
            separator,
            splitWorker,
            callback,
          );
      ...
      return interpreter.nativeToPseudo(
        nativeObj.apply(
          ...
          Array.prototype.slice
            .call(arguments)
            .map(i => interpreter.pseudoToNative(i)),
        ),
      );
    };
...
Interpreter.prototype.createGetter_ = function(func, left) {
  ...
  const state = new Interpreter.State(
    node,
...
Interpreter.prototype.stepCallExpression = function(stack, state, node) {
  ...
  if (!state.doneExec_) {
    ...
    if (func.eval) {
      ...
      const argsWithCallback = state.arguments_
